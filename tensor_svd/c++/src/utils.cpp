...
// Get kinetic energy operator for one spatial direction
void get_kinetic_operator(int N, Eigen::Tensor<double, 2>& A)
{
	for(int i = 0;i < N;i++)
	{
		A(i, i) = 2; // Fill diagonal
	}

	for(int i = 0;i < N - 1;i++)
	{
		A(i + 1, i) = - 1; // Fill subdiagonal
		A(i, i + 1) = - 1; // Fill superdiagonal
	}
}
